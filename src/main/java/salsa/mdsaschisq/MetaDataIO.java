...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	//            string usercomments = ManxcatCentral.config.Comment; // todo: saliya - handle comments afterwards
	//            if (usercomments != "")
	//            {
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	public static void WriteResults_Cluster(String ControlFileName, java.util.ArrayList linestooutput)
	{
		try
		{
			StreamWriter FileToWrite = null;

			if (!tangible.DotNetToJavaStringHelper.isNullOrEmpty(ControlFileName))
			{
			...
			if (FileToWrite != null)
			{
				...
				int LineLength = linestooutput.size();

				for (int countlines = 0; countlines < LineLength; countlines++)
				{
					FileToWrite.WriteLine(linestooutput.get(countlines));
				}
			}
			FileToWrite.Flush();
			FileToWrite.Close();
