...
public class Hotsun
{
	...
	{
		void invoke(String fname, int OutputOption, double[][] param, double[][] perr);
	}
	...
	{
		void invoke(double[][] DistributedVector, Desertwind Solution, boolean usexact, double[][] GlobalxVector, double[][] GlobalVectoronRight);
	}
	...
	public static boolean funcset = false; // If true the residuals are set in Desertwind
	public static boolean fullmatrixset = false; // if true set full matrices in Desertwind
	...
	public static double ChisqChangePerPoint = 0.001; // Stop when predicted change in Chisq per point <= ChisqChangePerPoint
	...
	public static int QLimitscalculationInterval = 6; //  recalculate Qhigh Qlow Trace and Norm after this number of iterations
	public static int InitialSteepestDescents = 0; // Start minimization with this number of Steepest Descent Iterations
	...
	public static boolean UseDiagonalScalinginSolvers = true; // Surprisingly Original Manxcat did NOT Scale in Solvers
	...
	public static boolean AddMarquardtQExplicitly = false; // If true add Marquardt explicitly in solvers (input variable)
	public static boolean derivtest = false; // If true do a derivative test
	public static boolean doingderivtest = false; // If true you are doing a derivative test
	public static boolean FullSecondDerivative = false; // If true use full second derivative matrix
	...
	public static boolean UseDiagonalScaling = false; // if true, scale diagonals in this computation (this is dynamic variable)
	public static boolean AddMarquardtQDynamically = false; // If true  add in Marquardt Factor in current computation (this is dynamic variable)
	...
	public static boolean DecomposeParameters = true;
	...
	public static Desertwind BeginningLinePositionSolution; // Set to starting point of search (holds param of starting point and value of first derivative there)
	public static Desertwind EndingLinePositionSolution; // Set to starting point of search (holds xshift from starting point and value and first derivative of end of line )
	...
	public static boolean[][] FixedParameter; // If true fix this parameter at initial value -- often zero -- Added 2009
	...
	public static boolean succ = false; // If true current fit reduced Chisq; if false it increased it
	...
	public static double Qlow = 1.0; //     Lower limit on Q (Estimate of minimum eigenvalue of scaled second derivative matrix)
	public static double Qhigh = 1.0; //    Upper limit on Q (Estimate of maximum eigenvalue of scaled second derivative matrix)
	...
	public static double dellim = 0.0; // Stop when predicted change in Chisq  <= dellim (calculated from ChisqChangePerPoint)
	...
	public static double pred2 = 0.0; //  Expected chisq change at next iteration from quadratic term in expansion using Chisq Approximation
	public static double pred3 = 0.0; //  Expected chisq change at next iteration from quadratic term in expansion using FULL formula
	...
	public static int CoordinateWriteFrequency = 80; // Frequency, in iterations, with which to write out MDS coordinates.

	public static void SetupManxcat()
	{
		//  Set Hotsun Input parameters
		maxit = ManxcatCentral.config.Maxit;
		nbadgo = ManxcatCentral.config.Nbadgo;
		ChisqChangePerPoint = ManxcatCentral.config.ChisqChangePerPoint;
		rho = ManxcatCentral.config.FletcherRho;
		Omega = ManxcatCentral.config.Omega;
		OmegaOption = ManxcatCentral.config.OmegaOption;
		sigma = ManxcatCentral.config.FletcherSigma;
		QHighInitialFactor = ManxcatCentral.config.QHighInitialFactor;
		QgoodReductionFactor = ManxcatCentral.config.QgoodReductionFactor;
		QLimitscalculationInterval = ManxcatCentral.config.QLimitscalculationInterval;
		InitialSteepestDescents = ManxcatCentral.config.InitialSteepestDescents;
		timmax = ManxcatCentral.config.TimeCutmillisec;
		CGResidualLimit = ManxcatCentral.config.CGResidualLimit;
		PowerIterationLimit = ManxcatCentral.config.PowerIterationLimit;
		eigenvaluechange = ManxcatCentral.config.Eigenvaluechange;
		eigenvectorchange = ManxcatCentral.config.Eigenvectorchange;
		extraprecision = ManxcatCentral.config.Extraprecision;
		addonforQcomputation = ManxcatCentral.config.AddonforQcomputation;
		derivtest = ManxcatCentral.config.Derivtest;
		dellim = ChisqChangePerPoint; // Set dellim for current size problem (original Manxcat didn't do this)
		CoordinateWriteFrequency = ManxcatCentral.config.CoordinateWriteFrequency;

		//  Parameters for Loop over Choices

		int NumberofPoints = Hotsun.Number_VectorParameters;
		InitializationLoops = ManxcatCentral.config.InitializationLoops;
		...
		FixedParameter = new boolean[NumberofPoints][];
		...
		int LocalNumberofPoints = Hotsun.Number_VectorParameters;
