package salsa.mdsaschisq;

public class ManxcatErrorMessages
{
	public static final String UnequalVectorLengths = "Vector lengths should be equal to call GetEuclideanDistance";

	public static final String TemplateStyleSheetNotAvailable = "Template style sheet Manxcat.Web.style is not available";

	public static final String UnidentifiedTagInHTMLGeneration = "Unidentified tag in HTML generation";
	public static String TemplateHtmlNotAvailable = "Template html file Manxcat.Web.template is not avaialable";
}