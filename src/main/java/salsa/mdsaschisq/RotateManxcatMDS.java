...
public class RotateManxcatMDS {
    ...
    public static void SetupRotateMDS() {
        ...
        if (SALSA_ProcessVariedandFixed.AreValuesSet(SALSAUtility.GlobalPointProperties)) {
            SALSAUtility.SALSAPrint(2,
                    " Reference Points Taken from basic Labels File " + ManxcatCentral.config.DataLabelsFileName);
        } else {
            ...
            tangible.RefObject<Integer> tempRef_InitializationFileType = new tangible.RefObject<Integer>(
                    InitializationFileType);
            tangible.RefObject<SALSADataPointProperties[]> tempRef_InitializationPointProperties = new tangible.RefObject<SALSADataPointProperties[]>(
                    InitializationPointProperties);
            tangible.RefObject<Integer> tempRef_InitializationNumberofPoints = new tangible.RefObject<Integer>(
            ...
            SALSA_Properties.ReadDataPointFile(OriginalMDSFileName, tempRef_InitializationFileType,
                    ...
                    tempRef_InitializationNumberofPoints);
        ...
        if (RotationOption_GenerateTestInput == 0) {
            ...
            tangible.RefObject<SALSADataPointProperties[]> tempRef_RotationPointProperties = new tangible.RefObject<SALSADataPointProperties[]>(
                    RotationPointProperties);
            tangible.RefObject<Integer> tempRef_RotationNumberofPoints = new tangible.RefObject<Integer>(
        ...
        SALSAUtility.SALSAPrint(0,
                " First Scale " + String.format("%.4E", FirstScale) + " Second Scale " + String.format("%.4E",
    ...
    public static boolean Calcfg(Desertwind Solution) throws MPIException { // call Accum with value and derivative for each point
        ...
        if (PointVectorDimension == 3) {
            for (int LocalVectorIndex = 0; LocalVectorIndex < PointVectorDimension; LocalVectorIndex++) {
                Solution.param[LocalVectorIndex + PointVectorDimension][0] = ProperAngle(
        ...
        if (PointVectorDimension == 3) {
            ...
            SetTotalRotation(DerivRotationScale[PointVectorDimension + 1], Scale, CurrentCosmicScaling, SubRotation[2],
                    SubDeriveRotation[1], SubRotation[0]); // Rotation for Second Angle
        ...
        if (PointVectorDimension == 2) {
            ...
            SetTotalRotation(DerivRotationScale[ScalePosition], DerivScale, CurrentCosmicScaling,
                    SubRotation[0]); // Rotation for Scale
        }
    ...
    public static void SetRotationMatrix(double[][] RotationMatrix, int FixedAxis, double Theta,
    ...
    public static void SetTotalRotation(double[][] RTotal, double Scaling, double[][] Cosmic, double[][] R3,
                                        double[][] R2, double[][] R1, boolean scale) {
    ...
    public static void InitializeParameters(Desertwind Solution, int CountStartingPoints) throws MPIException {
        ...
        for (int LocalVectorIndex1 = 0; LocalVectorIndex1 < PointVectorDimension; LocalVectorIndex1++) {
            System.arraycopy(CurrentCosmicScaling[LocalVectorIndex1], 0,
                    SavedCosmicScaling[CountStartingPoints][LocalVectorIndex1], 0, PointVectorDimension);
        }
        ...
        if (SALSAUtility.MPI_Size > 1) {
            ...
            // Note - MPI Call - double[]
            SALSAUtility.mpiOps.broadcast(RandomAngles, 0);
    ...
    public static void WriteRotateMDS(String fname, int OutputOption, double[][] param, double[][] perr) {
        ...
        for (int LocalVectorIndex1 = 0; LocalVectorIndex1 < PointVectorDimension; LocalVectorIndex1++) {
            System.arraycopy(SavedCosmicScaling[Hotsun.BestChisqLoop][LocalVectorIndex1], 0,
                    CurrentCosmicScaling[LocalVectorIndex1], 0, PointVectorDimension);
        }
        ...
        if (OutputOption == 0) {
            ...
            RotationFileProperties.GroupName = "MDS Run:" + config.RunNumber;
            ...
            for (int Cosmicdiagonals = 0; Cosmicdiagonals < PointVectorDimension; Cosmicdiagonals++) {
                RotationFileProperties.RotationParameters += " " + String.format("%.4E",
            ...
            OldComment += "MDS ROTATION Run " + config.RunNumber + " Start Time " + SALSAUtility.startTime + " *** ";
            OldComment += "\n RotationOption " + config.RotationOption + " First Scale " + String.format("%.4E",
                    FirstScale) + " Second Scale " + (new Double(SecondScale)).toString() + "\n First Mean";
            ...
            SALSA_Properties.WriteDataPointFile(fname, ManxcatCentral.config.Write2Das3D, "all ",
                    RotationFileProperties, RotationPointProperties, SALSAUtility.NumberOriginalPoints);
            return;
        }
        ...
        if (!Strings.isNullOrEmpty(fname)) {
            try (PrintWriter sw = new PrintWriter(
                    Files.newBufferedWriter(Paths.get(fname), Charset.defaultCharset()))) {
    ...
    public static void SetupFinalTransforamation(double[][] param, double[] Translation,
                                                 double[][] Rotation) { // Calculate Transformation without derivations
