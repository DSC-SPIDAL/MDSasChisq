...
public class ManxcatMDSDataProcessing
{
	...
	{
		if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("traditionaldirectory"))
		{
			UpdateManxcatMDS_Option12_TraditionalDirectory();
			return;
		}

		if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("cluster"))
		{
			UpdateManxcatMDS_Option12_Cluster();
			return;
		}

		if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("cutfile"))
		{
			UpdateManxcatMDS_Option12_FamilybyCuts();
			return;
		}

		if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("function"))
		{
			if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("statistics"))
			{
	...
	public static void UpdateManxcatMDS_Option12_Functions(boolean statistics)
	{
		...
		{ // Set up statistics by reading MDS file
			TypicalMDSFileName = ManxcatCentral.ResultDirectoryName + "\\SIMPLE" + ManxcatCentral.Configuration.ReducedVectorOutputFileName;
...
//				using (StreamReader sr = File.OpenText(FunctionFileName))
			...
			String labelFileDirectory = ManxcatCentral.Configuration.ClusterDirectory; // Directory for Colors attached to Points
	...
	public static void UpdateManxcatMDS_Option12_FamilybyCuts()
	{
		...
		if (ManxcatCentral.Configuration.ConversionOption.toLowerCase().contains("family2"))
		{
			FamilyNumber = 2;
		}
		...
		SALSALibrary.SALSA_Properties.WriteDataPointFile(labelfilename, ManxcatCentral.Configuration.Write2Das3D, "colon,SameFileName ", SALSAUtility.GlobalFileProperties, SALSAUtility.GlobalPointProperties, SALSAUtility.NumberOriginalPoints);
	...
	public static void UpdateManxcatMDS_Option12_TraditionalDirectory()
	{
		...
		String MDSandClusterDirectory = labelFileDirectory + "\\" + ManxcatCentral.Configuration.RunSetLabel + "-R" + ManxcatCentral.Configuration.RunNumber.toString() + "-ManxcatMDS"; // Output Directory
