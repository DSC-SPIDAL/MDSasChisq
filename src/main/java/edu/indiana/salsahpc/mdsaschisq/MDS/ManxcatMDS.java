...
public class ManxcatMDS
{
	...
	public static void Sequel()
	{
		...
		if (SALSAUtility.MPI_Rank == 0)
		{
			...
			String outDir = (new java.io.File(ManxcatCentral.Configuration.ReducedVectorOutputFileName)).getParent();
...
//			using (StreamReader sr = File.OpenText(WeightFileName))
	...
	{
		if (ManxcatCentral.Configuration.InitializationOption != 1 || !(new java.io.File(ManxcatCentral.Configuration.InitializationFileName)).isFile())
		{
			throw new RuntimeException("Simple initialization file necessary with the specified processing option");
		}

//C# TO JAVA CONVERTER NOTE: The following 'using' block is replaced by its Java equivalent:
//		using (StreamReader reader = new StreamReader(File.Open(ManxcatCentral.Configuration.InitializationFileName, FileMode.Open, FileAccess.Read, FileShare.Read)))
		StreamReader reader = new StreamReader(File.Open(ManxcatCentral.Configuration.InitializationFileName, FileMode.Open, FileAccess.Read, FileShare.Read));
		try
		{
			char[] sep = new char[] {' ', '\t'};
			String[] splits;
