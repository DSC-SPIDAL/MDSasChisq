...
public class ManxcatCentral
{
	...
	static void main(String[] args)
	{
		...
		try
		{
			...
			{
				MDS.ManxcatMDS.SetupHotsunforMDS();
			}

			else if (ProcessingOption <= 200)
			{
				RotateMDS.RotateManxcatMDS.SetupHotsunforRotateMDS();
			}
			Hotsun.SetupManxcat();
			...
			{
				++ManxcatCentral.Configuration.RunNumber;
			}

			//  Construct One Line Label
			Configuration.Pattern = SALSAUtility.ParallelPattern;
			SALSAUtility.PatternLabel = String.format("==== MDS %1$s ==== Option:%2$s PointCount_Global:%3$s ==== Run %4$s %5$s == File %6$s == %7$s ==== ", SALSAUtility.ParallelPattern, ManxcatCentral.Configuration.ProcessingOption, SALSAUtility.PointCount_Global, ManxcatCentral.Configuration.RunSetLabel, ManxcatCentral.Configuration.RunNumber, ManxcatCentral.Configuration.DistanceMatrixFile, SALSAUtility.startTime.ToLocalTime());
			SALSAUtility.SALSAPrint(0, SALSAUtility.PatternLabel);


			String startstring = "";

			if (!ManxcatCentral.Configuration.Comment.equals(""))
			{
				startstring = "\n";
			}
			ManxcatCentral.Configuration.Comment += startstring + SALSAUtility.PatternLabel;
			...
			{
				ManxcatCentral.Configuration.ResultDirectoryExtension = String.format("ManxcatMDS %1$s %2$s P[%3$s]", ManxcatCentral.Configuration.RunSetLabel, timestamp, pattern);
				ManxcatCentral.ResultDirectoryName = ManxcatCentral.Configuration.BaseResultDirectoryName + "\\" + ManxcatCentral.Configuration.ResultDirectoryExtension;
			}
			...
			if (ProcessingOption == 12)
			{
				...
				if ((SALSAUtility.MPIIOStrategy > 1) || (SALSAUtility.MPI_Rank == 0))
				{
					...
					SALSAUtility.SALSAUpdateMetaData(ManxcatCentral.Configuration);
			...
			{
				ManxcatCentral.ManxcatControl(new Hotsun.Hotsun.CalcfgSignature(MDS.ManxcatMDS.Calcfg), new Hotsun.Hotsun.IntializeSignature(MDS.ManxcatMDS.SetupMDSasChisq), new Hotsun.Hotsun.InitializeParametersSignature(MDS.ManxcatMDS.InitializeParameters), new Hotsun.Hotsun.SolveMatrixSignature(MDS.ManxcatMDS.SolveMatrix), new Hotsun.Hotsun.WriteSolutionSignature(MDS.ManxcatMDSWriteSolution.WriteMDS), new Hotsun.Hotsun.FindQlimitsSignature(MDS.ManxcatMDS.FindQlimits), new Hotsun.Hotsun.GlobalMatrixVectorProductSignature(MDS.ManxcatMDS.GlobalMatrixVectorProduct), new Hotsun.Hotsun.SequelSignature(MDS.ManxcatMDS.Sequel));
			...
			else if (ProcessingOption <= 100)
			{
				...
				MDS.ManxcatMDS.SetupMDSasChisq();
				// fill hotsun.globalparam with initialization file
				MDS.ManxcatMDS.FillupHotsun();
...
//                    ManxcatMDSRoutines.InitializeParameters(Hotsun.CurrentSolution, Hotsun.InitializationLoops);
				MDS.ManxcatMDS.Sequel();
			}
			else if (ProcessingOption <= 200)
			{
				ManxcatCentral.ManxcatControl(new Hotsun.Hotsun.CalcfgSignature(RotateMDS.RotateManxcatMDS.Calcfg), new Hotsun.Hotsun.IntializeSignature(RotateMDS.RotateManxcatMDS.SetupRotateMDS), new Hotsun.Hotsun.InitializeParametersSignature(RotateMDS.RotateManxcatMDS.InitializeParameters), new Hotsun.Hotsun.SolveMatrixSignature(GenericManxcat.SolveMatrix), new Hotsun.Hotsun.WriteSolutionSignature(RotateMDS.RotateManxcatMDS.WriteRotateMDS), new Hotsun.Hotsun.FindQlimitsSignature(GenericManxcat.FindQlimits), new Hotsun.Hotsun.GlobalMatrixVectorProductSignature(GenericManxcat.GlobalMatrixVectorProduct), new Hotsun.Hotsun.SequelSignature(RotateMDS.RotateManxcatMDS.Sequel));
			}
			...
			if ((SALSAUtility.MPIIOStrategy > 0) || (SALSAUtility.MPI_Rank == 0))
			{
				...
				SALSAUtility.WriteTiming_Cluster(Configuration.TimingOutputFileName, Configuration.RunSetLabel, Configuration.RunNumber, Configuration.DistanceMatrixFile, MPI.Environment.getProcessorName());
	...
	private static void ReadConfiguration()
	{
		...
		if (!tangible.DotNetToJavaStringHelper.isNullOrEmpty(Configuration.SelectedClusters) && !"none".equals(Configuration.SelectedClusters))
		{
			char[] sep = new char[] {','};
			String[] splits = Configuration.SelectedClusters.trim().split(java.util.regex.Pattern.quote(sep.toString()), -1);
//C# TO JAVA CONVERTER TODO TASK: There is no equivalent to implicit typing in Java:
		...
		SALSAUtility.ManxcatRunName = !tangible.DotNetToJavaStringHelper.isNullOrEmpty(Configuration.ManxcatRunName) ? Configuration.ManxcatRunName : "Unspecified Run";
		SALSAUtility.ManxcatRunDescription = !tangible.DotNetToJavaStringHelper.isNullOrEmpty(Configuration.ManxcatRunDescription) ? Configuration.ManxcatRunDescription : "Description not specified";
	}
	...
	public static void WriteCoordinates(int Outputtype, Hotsun.WriteSolutionSignature WriteSolution)
	{
		...
		if (SALSAUtility.MPI_Rank == 0)
		{
			...
			String labelfilename = ManxcatCentral.Configuration.ReducedVectorOutputFileName;
...
//                else
	...
	public static void launchQlimits(Hotsun.FindQlimitsSignature FindQlimits)
	{
		if ((Hotsun.numit == 0) || (Hotsun.numit >= ManxcatCentral.IterationtorecalculateQLimits))
		{
			...
			if (ManxcatCentral.Configuration.FullSecondDerivativeOption == 1)
			{
				Hotsun.FullSecondDerivative = true;
			}
