...
public class SALSA_ProcessVariedandFixed
{
	...
	{ // Set up Load Balanced Fixed and Varied Points
		...
		{
			OriginalPropertiesfilename = ManxcatCentral.Configuration.ControlDirectoryName + "\\" + OriginalPropertiesfilename;
		}
		...
		if ((new java.io.File(OriginalPropertiesfilename)).isFile())
		{
			...
			for (int OriginalPointIndex = 0; OriginalPointIndex < SALSAUtility.NumberOriginalPoints; OriginalPointIndex++)
			{
				SALSAUtility.OriginalDataLabels[OriginalPointIndex] = SALSAUtility.OriginalPointProperties[OriginalPointIndex].grouplabel + ":" + (new Integer(SALSAUtility.OriginalPointProperties[OriginalPointIndex].group)).toString();
			}
		}
		else
		{
...
//  Process special case where all varied -- this is default scenario
		if ((ManxcatCentral.Configuration.FixedPointCriterion.equals("none")) && (ManxcatCentral.Configuration.VariedPointCriterion.equals("all")))
		...
		if (!VariedCollection.equals("all") && !VariedCollection.equals("rest"))
		{
			...
			if (SALSAUtility.MPI_Rank == 0)
			{
				SALSAUtility.SALSAPrint(0, " Varied Point Selection Categories: " + ToDecode + " Method: " + VariedCollection + " File " + ManxcatCentral.Configuration.Selectedvariedpointfile);
			}
		...
		{ // Cases originalpoint family1 family2 cluster group allinfile
			...
			if (SALSAUtility.MPI_Rank == 0)
			{
				SALSAUtility.SALSAPrint(0, " Fixed Point Selection Categories: " + ToDecode + " Method: " + FixedCollection + " File " + ManxcatCentral.Configuration.Selectedfixedpointfile);
			}
		...
		if (SALSAUtility.MPI_Rank == 0)
		{
			if (missingdata > 0)
			{
				SALSAUtility.SALSAPrint(0, " At this stage missing initial vales for " + (new Integer(missingdata)).toString() + " Points -- must be in initialization file");
			}

			int NotUsed = SALSAUtility.NumberOriginalPoints - SALSAUtility.NumberVariedPoints - SALSAUtility.NumberFixedPoints;
			SALSAUtility.SALSAPrint(0, " Number of Varied Points " + (new Integer(SALSAUtility.NumberVariedPoints)).toString() + " Fixed Points " + (new Integer(SALSAUtility.NumberFixedPoints)).toString() + " Not Used " + NotUsed);
		}
		return;
	...
	public static void SetDataLabels(String[] GlobalDataLabels)
	{
		...
		{
			GlobalDataLabels[GlobalIndex] = (new Integer(GlobalIndex)).toString();
		}
